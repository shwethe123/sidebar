...
import { db1, serverTimestamp } from "@/firebase/config"
...
export default {
  setup() {
    ...
    const addPost = async () => {
      ...
      const res = await db1.collection("g_posts").add(newPost)
      console.log(res)
      router.push("/")
    }

    return {
      name, id_name, nrc, address, phone, loca, department, work,
      addPost, birthDay, birthMonth, birthYear, days, months, years
    }
  }
}
</script>
