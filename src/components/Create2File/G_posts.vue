...
import { db2, serverTimestamp } from "@/firebase/config"
...
export default {
  setup() {
    ...
    const addPost = async () => {
      if (!name.value || !id_name.value || !nrc.value || !address.value || !phone.value || !loca.value || !department.value || !work.value || !birthDay.value || !birthMonth.value || !birthYear.value) {
      ...
      try {
        const res = await db2.collection("g_posts").add(newPost)
        console.log(res)
        router.push("/")
      } catch (error) {
