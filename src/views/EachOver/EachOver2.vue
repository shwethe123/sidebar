<template>
  ...
  <div class="table-responsive">
    ...
    <div class="form_box">
      <div >
          ...
          <div v-for="post in postsDb2" :key="post">
            <router-link class="router" :to="{name:'Detail',params: {id:post.id}}">
              <p :class="{ false_p: !post.condition }">
                {{ post.name }}
              </p>
            </router-link>
          </div>
        ...
        <div>
          ...
          <div v-for="post in sales_inDb2" :key="post">
            <router-link class="router" :to="{name:'Detail',params: {id:post.id}}">
              <p :class="{ false_p: !post.condition }">{{ post.name }}</p>
            </router-link>
          </div>
        </div>
        ...
        <!-- <div>
          <h2>အပြင်သွား</h2>
          <div v-for="post in out_JutyDb2" :key="post">
            <router-link class="router" :to="{name:'Detail',params: {id:post.id}}">
              <p>{{ post.name }}</p>
            </router-link>
          </div>
    ...
    <!-- <div>
      <FormsOne :s_post="posts"></FormsOne>
    </div>
    <div>
...
export default {
  ...
  setup() {
    ...
    load();
    return {sales_inDb2,postsDb2,sales_outDb2,order_inDb2,error,accountingDb2,auditDb2 ,car_orderDb2 ,out_JutyDb2}
  }
}
</script>
